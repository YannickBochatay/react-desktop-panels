  ...
  _createClass(Panel, [{
    ...
    value: function handleDrag(e) {
      ...
      if (vertical) {
        ...
        var width = Math.min(Math.max(minSize, this.widthInit + deltaX), maxSize);

        this.setState({ width: width });
      } else {

        var deltaY = (e.pageY - this.yClick) * (resizerPos === "before" ? -1 : 1);
        var height = Math.min(Math.max(minSize, this.heightInit + deltaY), maxSize);
...
Panel.propTypes = {
  ...
  fixedSize: _react.PropTypes.bool
};
