...
var Resizable = function (_Component) {
    ...
    _createClass(Resizable, [{
        ...
        value: function getNode() {
            ...
            return node.getBoundingClientRect ? node : _reactDom2.default.findDOMNode(node);
        }
    }, {
        key: "getZoomFactor",
        ...
        value: function getComputedDim() {
            ...
            if (unit === "%" && this.unit === "%") {
                ...
                var borderTop = parseFloat(parentStyle.borderTopWidth);
                var borderBottom = parseFloat(parentStyle.borderBottomWidth);
                var borderLeft = parseFloat(parentStyle.borderLeftWidth);
                var borderRight = parseFloat(parentStyle.borderRightWidth);

                var widthParent = parentWidth - paddingLeft - paddingRight - borderLeft - borderRight;
                var heightParent = parentHeight - paddingTop - paddingBottom - borderTop - borderBottom;
        ...
        value: function handleDrag(e) {
            ...
            if (direction === "row") {
                ...
                var offset = (e.pageX - this.xClick) * this.zoomFactor.x;
                var deltaX = offset * (resizerPos === "before" ? -1 : 1);

                size = Math.min(maxSize, Math.max(minSize, this.sizeInit + deltaX));
            } else {

                var _offset = (e.pageY - this.yClick) * this.zoomFactor.y;
                var deltaY = _offset * (resizerPos === "before" ? -1 : 1);
